...
const TrendingItem: React.FC<TrendingItemProps> = ({ item }) => {
  ...
  const { setLiveId, setToken, setTokenCreateDate, setLiveData } = useSnapshot(liveStore);

  useEffect(() => {
    // Generate the thumbnail on component mount
    createThumbnail({
      url: getFullImageUrl(item?.live?.recordUrl) || "",
      timeStamp: 200,
    })
      .then(response => setThumbnailUrl(response.path))
